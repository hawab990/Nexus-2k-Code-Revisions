...
void Utilities:: drawPossibleMoves(Graphics^ graphics)
{
	...
	//The dictionary associates the index value with the  data type bool. Meaning if you give it a point, it will always return the
	//Data type bool.
	Dictionary<Point, bool>^ visited = gcnew Dictionary<Point, bool>();
	//Matching the picked cell with the bool true.
	...
	while(frontier->Count > 0)
	{
		...
		//Gets the point thats going to be removed from the current cell and inspects its neighbours.
		for each(Point next in neighbors(current))
		{
			...
			{
				// if the cells neighbours have not been visited, add them to the que.
...
// This method passes in all the points that would be inside the cell adjacent to the to toCheck
List<Point>^ Utilities::neighbors (Point toCheck)
{
	...
	{
		//This checks if each cell inside the grid.
		...
		// This checks if the cell can be moved to.
...
{
	List<Point>^ toReturn = gcnew List<Point>();
	// if there is nothing saved in the pickedCell 
	if(pickedCell == nullptr) 
	{
	...
	}// After the list of the possible path, this gets all the necessary steps to traverse from start to finish
	//This  checks if you you dont have  a path from start to finish, then return an empty list.
	if(!cameFrom->ContainsKey( Point(endPoint->X, endPoint->Y)) )
	{
		return toReturn;
	}
	{	
		//After it has found a path, it will assemble the necessary steps to get there.
